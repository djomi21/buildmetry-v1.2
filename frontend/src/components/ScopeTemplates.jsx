...
export default function ScopeTemplates({ templates, showToast, db }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div className="g4">
        <KpiCard label="Total Templates" val={templates.length} sub="" color="#63b3ed"/>
      </div>
        ...
        <div className="ov" onClick={e => e.target === e.currentTarget && setForm(null)}>
          <div className="mo" style={{ maxWidth: 540, marginTop: 60 }}>
            <div style={{ padding: "17px 24px", borderBottom: "1px solid var(--border-2)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 17, fontWeight: 800 }}>{form._id ? "Edit Scope Template" : "Add Scope Template"}</div>
            ...
            <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 13 }}>
              <div>
                ...
                <input className="inp" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Standard Residential Remodel"/>
              </div>
              <div>
                <label className="lbl">Content</label>
                <textarea className="inp" value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} rows={8} placeholder="Describe the scope of work…" style={{ resize: "vertical" }}/>
              </div>
